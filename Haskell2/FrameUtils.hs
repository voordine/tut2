...
class (FrameType a) => Checkable a where
 infix 6 .<
 (.<) :: a -> a  -> Bool
 (.=) :: a -> a  -> Bool
 supersets :: a -> [a]
 subsets  :: a -> [a]
 supersets f = [ g | g <- functionSpaceOf f , f .< g ]
 subsets   f = [ g | g <- functionSpaceOf f , g .< f ]
 a .= b = a .< b && b .< a


instance Checkable E where
  a .< b = a == b
instance Checkable W where
  a .< b = a == b
instance Checkable T where
  a .< b = a ==> b
instance Checkable Z where
  a .< b = a == b
instance (Checkable a,Checkable b )
       => Checkable (a->b) where
...
instance Show Denotation where
  show (Sem x) = reverse $ clean [] $ concat [ "" , showcast typeRep ] where
      ...
      showcast t
          ...
          | t == typeEET = show $ fromJust$  ((fromDynamic x) :: Maybe (E -> E -> T))
          | t == typeETT  = showFunctionAsSet $fromJust$  ((fromDynamic x) :: Maybe ((E->T)->T))
          | t == typeListE  = show $fromJust$  ((fromDynamic x) :: Maybe ([E]))
          | otherwise =  "~"
